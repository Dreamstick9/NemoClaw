...
Shields must be down for `mcp restart`, and a rebuild that started with shields up restores them to UP.
Run `$$nemoclaw <sandbox-name> shields down --timeout 15m --reason "MCP maintenance"` before that restart, then `$$nemoclaw <sandbox-name> shields up` after it succeeds.
